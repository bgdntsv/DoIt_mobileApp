...
export const ExerciseType = () => {
    ...
    const route =
        useRoute<RouteProp<HOME_STACK_ROUTE_PROPS, 'Exercises-type'>>()
    ...
    useEffect(() => {
        if (isFocused) {
            navigation
                .getParent()
                ?.setOptions({ headerTitle: t('select_exercise') })
        }
    }, [isFocused])
    ...
    const getContent = useCallback(() => {
        const exercisesToShow = exercises
            .filter((e) =>
                e.muscleArea.find((m) => m.includes(exercisesType, 0))
            )
            .filter((e) =>

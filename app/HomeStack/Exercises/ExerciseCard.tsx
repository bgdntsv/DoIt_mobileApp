...
export const ExerciseCard = ({ exercise, type, select = false }: propTypes) => {
    ...
    const getPlaceText = (): string => {
        return `${exercise.gym ? t('gym') : ''}${
            exercise.home || exercise.outdoors ? ', ' : ''
        }${exercise.home ? t('home') : ''}${exercise.outdoors ? ', ' : ''}${
            exercise.outdoors ? t('outdoors') : ''
        }`
    }
        ...
        <Pressable style={styles.container} onPress={cardClick}>
            ...
            {(exercise.gym || exercise.outdoors || exercise.home) && (
                <Text style={styles.text}>
                    <Text style={styles.subTitle}>{t('place')}:</Text>{' '}
                    {getPlaceText().toLowerCase()}
                </Text>
            )}
            ...
            <Text style={styles.text}>
                <Text style={styles.subTitle}>{t('muscle_area')}:</Text>{' '}
                {muscleAreaArrayShow().toLowerCase()}
            </Text>
